...
class FasteryShippingMethod extends WC_Shipping_Method
{
    ...
    public function calculate_shipping($package)
    {
        ...
        // Почта России
        if (isset($items->mail) && !empty($items->mail->cost)) {
        ...
        // Самовывоз
        $ignet_fastery_cost = isset($_COOKIE['fastery-cost']) ? $_COOKIE['fastery-cost'] : WC()->session->get('ignet_fastery_cost');
        ...
        if (isset($items->points)) {
            $rate['cost'] = '';
            $uid = isset($_COOKIE['fastery-uid']) ? $_COOKIE['fastery-uid'] : WC()->session->get('ignet_fastery_uid');
